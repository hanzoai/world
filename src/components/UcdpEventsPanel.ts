...
export class UcdpEventsPanel extends Panel {
  ...
  private renderContent(): void {
    ...
    const tabsHtml = this.renderTabs(
      tabs.map((tb) => ({ key: tb.key, label: tb.label, count: tabCounts[tb.key] })),
      this.activeTab,
      (key) => {
        this.activeTab = key as UcdpEventType;
        this.renderContent();
      },
    );
    ...
    if (displayed.length === 0) {
      bodyHtml = this.emptyStateHtml(t('common.noEventsInCategory'));
    } else {
