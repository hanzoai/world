...
export function renderNotificationsSettings(host: NotificationsSettingsHost): NotificationsSettingsResult {
  ...
  if (isPro) {
    ...
    html += `<div class="wm-pref-group-content wm-notif-tab-content">`;
    html += `<div class="ai-flow-toggle-desc">Get real-time intelligence alerts delivered to WhatsApp, Telegram, Slack, Discord, and Email with configurable sensitivity, quiet hours, and digest scheduling.</div>`;
  ...
  return {
    ...
    attach(container: HTMLElement): () => void {
      ...
      const CHANNEL_LABELS: Record<ChannelType, string> = { telegram: 'Telegram', whatsapp: 'WhatsApp', email: 'Email', slack: 'Slack', discord: 'Discord', webhook: 'Webhook', web_push: 'Browser push' };
      ...
      function renderChannelRow(channel: NotificationChannel | null, type: ChannelType): string {
        ...
        if (channel?.verified) {
          ...
          if (type === 'telegram') {
            ...
            sub = escapeHtml(channel.phoneNumber ?? 'connected');
          } else if (type === 'email') {
        ...
        if (type === 'whatsapp') {
          return `<div class="us-notif-ch-row" data-channel-type="whatsapp">
            <div class="us-notif-ch-icon">${icon}</div>
            <div class="us-notif-ch-body">
              <div class="us-notif-ch-name">${name}</div>
              <div class="us-notif-ch-sub">Get daily intelligence briefs on WhatsApp</div>
            </div>
            <div class="us-notif-ch-actions">
              <button type="button" class="us-notif-ch-btn us-notif-ch-btn-primary" id="usConnectWhatsApp">Add Number</button>
            </div>
          </div>`;
        }
      ...
      function renderNotifContent(data: Awaited<ReturnType<typeof getChannelsData>>): string {
        const channelTypes: ChannelType[] = ['telegram', 'whatsapp', 'email', 'slack', 'discord', 'webhook'];
      ...
      container.addEventListener('click', (e) => {
        ...
        if (target.closest('#usConnectWhatsApp')) {
          const rowEl = target.closest('.us-notif-ch-row') as HTMLElement | null;
          if (!rowEl) return;
          rowEl.querySelector('.us-notif-ch-actions')!.innerHTML = `
            <form class="us-notif-wa-form" style="display:flex;gap:6px;align-items:center">
              <input type="tel" class="us-notif-wa-input" placeholder="+1 234 567 8901" style="width:160px;padding:4px 8px;background:var(--input-bg);border:1px solid var(--border);color:var(--text);font-size:12px;border-radius:3px" />
              <button type="submit" class="us-notif-ch-btn us-notif-ch-btn-primary" style="white-space:nowrap">Save</button>
            </form>`;
          const form = rowEl.querySelector('.us-notif-wa-form') as HTMLFormElement;
          const input = rowEl.querySelector('.us-notif-wa-input') as HTMLInputElement;
          input.focus();
          form.addEventListener('submit', async (e) => {
            ...
            if (!phone || phone.length < 8) {
              rowEl.querySelector('.us-notif-error')?.remove();
              rowEl.insertAdjacentHTML('beforeend', '<span class="us-notif-error">Enter a valid phone number with country code</span>');
              return;
            }
            const { setWhatsAppChannel } = await import('@/services/notification-channels');
            try {
              await setWhatsAppChannel(phone);
              if (!signal.aborted) { saveRuleWithNewChannel('whatsapp'); reloadNotifSection(); }
            } catch {
              rowEl.querySelector('.us-notif-error')?.remove();
              rowEl.insertAdjacentHTML('beforeend', '<span class="us-notif-error">Failed to save number</span>');
            }
          });
          return;
        }
      ...
      const onMessage = (e: MessageEvent): void => {
        const trustedOrigin = e.origin === window.location.origin ||
          e.origin === 'https://world.hanzo.ai' ||
          e.origin === 'https://www.world.hanzo.ai' ||
          e.origin.endsWith('.world.hanzo.ai');
