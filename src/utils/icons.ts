// Canonical Hanzo iconography.
//
// @hanzo/ui (and the platform / console apps) use lucide app-wide via
// `lucide-react`. Hanzo World is framework-free vanilla TS, so we inline the SAME
// lucide glyphs here — one design language across every Hanzo surface, no React,
// no runtime dependency. Markup is verbatim from lucide 0.469.0 (the version
// pinned in ~/work/hanzo/platform). Every icon shares the lucide wrapper:
// 24×24, fill none, stroke currentColor, width 2, round caps/joins — so it
// inherits color + size from context exactly like the switcher's existing cloud
// glyph did.
