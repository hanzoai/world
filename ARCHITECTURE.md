...
Detected by hostname (`tech.world.hanzo.ai` → tech, `finance.world.hanzo.ai` → finance, etc.) or localStorage on desktop. Controls: default panels, map layers, refresh intervals, theme, UI text. Variant change resets all settings to defaults.
...
| File | Purpose |
|------|---------|
| `_cors.js` | Origin allowlist (world.hanzo.ai, Vercel previews, tauri://localhost, localhost) |
