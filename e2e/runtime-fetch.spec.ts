...
test.describe('desktop runtime routing guardrails', () => {
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('world.hanzo.ai/api/fred-data')) {
        ...
        if (url.includes('world.hanzo.ai/api/stablecoin-markets')) {
    ...
    expect(result.calls.some((url) => url.includes('world.hanzo.ai/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('world.hanzo.ai/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('world.hanzo.ai/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('world.hanzo.ai/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('world.hanzo.ai/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('world.hanzo.ai/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    expect(result.macArm).toBe('https://world.hanzo.ai/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://world.hanzo.ai/api/download?platform=windows-exe&variant=full');
  ...
  test('cloud fallback blocked without WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('world.hanzo.ai'));
  ...
  test('cloud fallback allowed with valid WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('world.hanzo.ai') && init?.headers) {
        ...
        if (url.includes('world.hanzo.ai/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('world.hanzo.ai')).length,
