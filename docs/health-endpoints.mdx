...
```bash
# Full health check
curl -s https://api.world.hanzo.ai/api/health | jq .

# Compact (problems only)
curl -s "https://api.world.hanzo.ai/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://api.world.hanzo.ai/api/health
...
```bash
curl -s https://api.world.hanzo.ai/api/seed-health \
  -H "Origin: https://world.hanzo.ai" | jq .
```
...
STATUS=$(curl -s "https://api.world.hanzo.ai/api/health?compact=1")
