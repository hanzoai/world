...
| URL | Use |
|-----|-----|
| `https://maps.world.hanzo.ai/<file>` | Production URL (CF-proxied, cached, CORS headers) |
...
2. `country-boundary-overrides.geojson` — optional higher-resolution polygons from [Natural Earth](https://www.naturalearthdata.com/), served from R2 CDN (`maps.world.hanzo.ai`). Features matched by `ISO3166-1-Alpha-2` (or `ISO_A2`) code; matching features replace the base geometry
...
PMTiles are also served from R2 via `maps.world.hanzo.ai`, configured through `VITE_PMTILES_URL`.

## Common Mistakes

| Mistake | Fix |
|---------|-----|
| Using `pub-*.r2.dev` URLs in code | Always use `maps.world.hanzo.ai` (CF-proxied) |
