...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `world.hanzo.ai`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
