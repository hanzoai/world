  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    if (bearerApiKey) {
      ...
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="worldmonitor", error="invalid_token", resource_metadata="https://api.world.hanzo.ai/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
  } else {
    ...
    if (!candidateKey) {
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="worldmonitor", resource_metadata="https://api.world.hanzo.ai/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
