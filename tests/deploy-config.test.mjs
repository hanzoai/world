...
describe('security header guardrails', () => {
  ...
  it('CSP script-src includes Clerk origin for auth UI', () => {
    ...
    const scriptSrc = csp.match(/script-src\s+([^;]+)/)?.[1] ?? '';
    assert.ok(
      scriptSrc.includes('clerk.accounts.dev') || scriptSrc.includes('clerk.world.hanzo.ai'),
  ...
  it('CSP frame-src includes Clerk origin for auth modals', () => {
    ...
    assert.ok(
      frameSrc.includes('clerk.accounts.dev') || frameSrc.includes('clerk.world.hanzo.ai'),
